# Lab 4-A: ODP 기반 Open Order Fact View & Analytic Model

## 목표

S/4HANA ODP DataSource를 Replication Flow로 복제한 **Local Table**을 소스로, 미결 판매 오더를 분석하는 **Fact View**와 **Analytic Model**을 개발합니다.

**소요 시간**: 약 60분

---

## 개발 오브젝트

| 오브젝트 | Technical Name | 소스 |
|---------|----------------|------|
| SQL View (Fact View) | `WS_HL_SQLV_OPEN_ORDER_ODP` | Local Tables (ODP 복제) |
| Analytic Model | `WS_RL_OPEN_ORDER_ODP` | `WS_HL_SQLV_OPEN_ORDER_ODP` |

---

## 소스 데이터 구조

Lab 2에서 Replication Flow로 복제된 Local Table 6개를 사용합니다.

```mermaid
erDiagram
    SAP_SD_IL_2LIS_11_VAHDR ||--o{ SAP_SD_IL_2LIS_11_VAITM : "VBELN"
    SAP_SD_IL_2LIS_11_VAITM ||--o{ SAP_SD_IL_2LIS_11_VASCL : "VBELN+POSNR"
    SAP_SD_IL_2LIS_11_VAITM ||--o{ SAP_SD_IL_2LIS_12_VCITM : "VBELN+POSNR"
    SAP_SD_IL_2LIS_11_VAITM ||--o{ SAP_SD_IL_2LIS_13_VDITM : "VBELN+POSNR"
    SAP_SD_IL_2LIS_13_VDITM }o--|| SAP_SD_IL_2LIS_13_VDHDR : "VBELN"

    SAP_SD_IL_2LIS_11_VAHDR {
        string VBELN PK "수주번호"
        string VKORG "판매조직"
        string BUKRS "회사코드"
        string VTWEG "유통채널"
        string AUART "전표유형"
        string KUNNR "주문자"
        string VKBUR "판매사무소"
        string VKGRP "판매그룹"
        string WAERK "전표통화"
        string HWAER "회사코드통화"
        date ERDAT "생성일"
        date VDATU "납품요청일(RDD)"
        string VBTYP "전표범주"
        string ROCANCEL "취소여부"
    }
    SAP_SD_IL_2LIS_11_VAITM {
        string VBELN PK,FK "수주번호"
        string POSNR PK "항목번호"
        string MATNR "자재번호"
        string MATKL "자재그룹"
        string SPART "제품군"
        string WERKS "플랜트"
        string MEINS "기본단위"
        decimal KWMENG "주문수량"
        string ABGRU "거부사유"
        string BZIRK "판매지구"
        string PSTYV "항목범주"
        string ROCANCEL "취소여부"
    }
    SAP_SD_IL_2LIS_11_VASCL {
        string VBELN PK,FK "수주번호"
        string POSNR PK,FK "항목번호"
        decimal BMENG "확정수량"
        string ROCANCEL "취소여부"
    }
    SAP_SD_IL_2LIS_12_VCITM {
        string VGBEL FK "수주번호(참조)"
        string VGPOS FK "수주항목(참조)"
        decimal LFIMG "출고수량"
        date WADAT_IST "실제출고일"
        string VGTYP "참조전표범주"
        string ROCANCEL "취소여부"
    }
    SAP_SD_IL_2LIS_13_VDITM {
        string AUBEL FK "수주번호(참조)"
        string AUPOS FK "수주항목(참조)"
        string VBELN FK "청구전표번호"
        decimal FKIMG "청구수량"
        decimal NETWR "청구금액"
        string AUTYP "참조전표범주"
        string VGTYP "참조전표범주2"
        string RFBSK "회계전기상태"
        string SFAKN "취소전표번호"
        string FKSTO "취소여부"
        string ROCANCEL "취소여부"
    }
    SAP_SD_IL_2LIS_13_VDHDR {
        string VBELN PK "청구전표번호"
        string FKART "청구유형"
        string ROCANCEL "취소여부"
    }
```

---

## Part A. Fact View 생성

### Step A-1. SQL View 생성

1. Data Builder → **New** → **SQL View**
2. 기본 속성 설정:

| 속성 | 값 |
|------|-----|
| Business Name | `Open Order ODP Fact View` |
| Technical Name | `WS_HL_SQLV_OPEN_ORDER_ODP` |
| Semantic Usage | `Fact` |

> **중요**: Semantic Usage를 **Fact**로 설정해야 이 View가 Analytic Model의 Fact Source로 인식됩니다.  
> Fact View에는 **Measure 타입 필드가 반드시 1개 이상** 있어야 합니다. 다음 Step A-3에서 수치 필드를 Measure로 전환합니다.

### Step A-2. SQL 작성

SQL Editor에 아래 쿼리를 입력합니다.

<details>
<summary>정답 보기 — 먼저 스스로 작성해 본 후 확인하세요</summary>

```sql
SELECT 
-- 헤더
    H.VBELN,
    H.ERDAT,
    TO_VARCHAR(H.ERDAT, 'YYYYMM')                       AS CalendarYearMonth,
    H.VDATU,
    H.VKORG,
    H.BUKRS,
    H.VTWEG,
    H.AUART,
    H.KUNNR,
    H.VKBUR,
    H.VKGRP,
    H.WAERK,
    H.HWAER,
    H.VBTYP, 
-- 아이템
    I.POSNR,
    I.MATNR,
    I.MATKL,
    I.SPART,
    I.WERKS,
    I.MEINS,
    I.PSTYV,
    I.ABGRU,
    I.BZIRK, 
-- 4대 지표
    I.KWMENG,
    COALESCE(C.CONFIRMED_QTY, 0)                        AS CONFIRMED_QTY,
    COALESCE(G.GI_QTY, 0)                               AS GI_QTY,
    G.GI_DATE,
    COALESCE(B.BILL_QTY, 0)                             AS BILL_QTY,
    COALESCE(B.BILL_AMT, 0)                             AS BILL_AMT, 
-- 파생 지표
    I.KWMENG - COALESCE(G.GI_QTY, 0)                   AS OPEN_DLV_QTY,
    COALESCE(G.GI_QTY, 0) - COALESCE(B.BILL_QTY, 0)   AS UNBILLED_QTY, 
-- (1) 납품진행상태
    CASE
        WHEN G.GI_DATE IS NULL THEN 'Not Started'
        WHEN (I.KWMENG - COALESCE(G.GI_QTY, 0)) > 0
             AND G.GI_DATE IS NOT NULL THEN 'In Progress'
        ELSE 'Completed'
    END                                                  AS DELIVERY_STATUS, 
-- (2) 비교기준일
    CASE
        WHEN G.GI_DATE IS NOT NULL
             AND (I.KWMENG - COALESCE(G.GI_QTY, 0)) <= 0 THEN G.GI_DATE
        ELSE CURRENT_DATE
    END                                                  AS COMPARISON_DATE, 
-- (3) Lead-time (납품요청일 기준)
    CASE
        WHEN G.GI_DATE IS NOT NULL
             AND (I.KWMENG - COALESCE(G.GI_QTY, 0)) <= 0
            THEN DAYS_BETWEEN(H.VDATU, G.GI_DATE)
        ELSE DAYS_BETWEEN(H.VDATU, CURRENT_DATE)
    END                                                  AS RDD_LEADTIME_DAYS, 
-- (4) RDD 준수여부
    CASE
        WHEN G.GI_DATE IS NOT NULL
             AND (I.KWMENG - COALESCE(G.GI_QTY, 0)) <= 0
            THEN CASE
                     WHEN DAYS_BETWEEN(H.VDATU, G.GI_DATE) <= 0 THEN 'On-Time'
                     ELSE 'Delay'
                 END
        ELSE CASE
                 WHEN DAYS_BETWEEN(H.VDATU, CURRENT_DATE) <= 0 THEN 'On-Time'
                 ELSE 'Delay'
             END
    END                                                  AS RDD_COMPLIANCE 

-- 수주 헤더 (메인)
FROM "SAP_CONTENT.SAP_SD_IL_2LIS_11_VAHDR" H 

-- 수주 아이템
    INNER JOIN "SAP_CONTENT.SAP_SD_IL_2LIS_11_VAITM" I
        ON H.VBELN = I.VBELN AND I.ROCANCEL = '' 

-- 확정수량 (납품일정행)
    LEFT JOIN (
        SELECT VBELN, POSNR,
               SUM(BMENG) AS CONFIRMED_QTY
        FROM "SAP_CONTENT.SAP_SD_IL_2LIS_11_VASCL"
        WHERE ROCANCEL = ''
        GROUP BY VBELN, POSNR
    ) C ON I.VBELN = C.VBELN AND I.POSNR = C.POSNR 

-- 출하수량 / 실출하일
    LEFT JOIN (
        SELECT VGBEL AS SO_VBELN,
               VGPOS AS SO_POSNR,
               SUM(LFIMG)       AS GI_QTY,
               MAX(WADAT_IST)   AS GI_DATE
        FROM "SAP_CONTENT.SAP_SD_IL_2LIS_12_VCITM"
        WHERE ROCANCEL = '' AND VGTYP = 'C'
        GROUP BY VGBEL, VGPOS
    ) G ON I.VBELN = G.SO_VBELN AND I.POSNR = G.SO_POSNR 

-- 청구수량 / 청구금액
    LEFT JOIN (
        SELECT I2.AUBEL AS SO_VBELN,
               I2.AUPOS AS SO_POSNR,
               SUM(CASE WHEN I2.AUTYP IN ('H','K') OR I2.VGTYP = 'T'
                        THEN I2.FKIMG * -1 ELSE I2.FKIMG END) AS BILL_QTY,
               SUM(CASE WHEN I2.AUTYP IN ('H','K') OR I2.VGTYP = 'T'
                        THEN I2.NETWR * -1 ELSE I2.NETWR END) AS BILL_AMT
        FROM "SAP_CONTENT.SAP_SD_IL_2LIS_13_VDITM" I2
            INNER JOIN "SAP_CONTENT.SAP_SD_IL_2LIS_13_VDHDR" H2 ON I2.VBELN = H2.VBELN
        WHERE I2.ROCANCEL = ''
          AND H2.ROCANCEL = ''
          AND H2.FKART NOT IN ('IV', 'IG')        -- Intercompany Invoice/Credit Memo 제외
          AND (I2.RFBSK IS NULL OR I2.RFBSK = '')
          AND (I2.SFAKN IS NULL OR I2.SFAKN = '')
          AND (I2.FKSTO IS NULL OR I2.FKSTO = '')
          AND I2.AUTYP = 'C'
        GROUP BY I2.AUBEL, I2.AUPOS
    ) B ON I.VBELN = B.SO_VBELN AND I.POSNR = B.SO_POSNR

-- 메인 WHERE절
WHERE H.VKORG NOT IN ('7600', '7700')   -- 제외 판매조직
  AND H.VBTYP = 'C'                     -- 수주 전표만
  AND H.ROCANCEL = ''                   -- 취소 제외
  AND (I.ABGRU = '' OR I.ABGRU IS NULL) -- 거부 제외
```

</details>

> 핵심 필터 포인트:
> - `ROCANCEL = ''` : 취소된 레코드 제외 (ODP 핵심 조건)
> - `VBTYP = 'C'` : 수주 전표 유형만 포함
> - `ABGRU = ''` : 거부되지 않은 아이템만

### Step A-3. 필드 속성 설정 (Semantic Type)

SQL 실행 후 Output 패널에서 각 필드의 Semantic Type을 설정합니다:

| 필드 | Semantic Type | Business Name |
|------|-------------|---------------|
| `VBELN` | Key | 영업전표번호 |
| `POSNR` | Key | 항목번호 |
| `ERDAT` | Attribute (Date) | 생성일 |
| `CalendarYearMonth` | Attribute | 생성년월 |
| `VDATU` | Attribute (Date) | 납품요청일 |
| `VKORG` | Attribute | 판매조직 |
| `BUKRS` | Attribute | 회사코드 |
| `KUNNR` | Attribute | 주문자 |
| `MATNR` | Attribute | 자재번호 |
| `KWMENG` | Measure | 주문수량 |
| `CONFIRMED_QTY` | Measure | 확정수량 |
| `GI_QTY` | Measure | 출고수량 |
| `BILL_QTY` | Measure | 청구수량 |
| `BILL_AMT` | Measure | 청구금액 |
| `OPEN_DLV_QTY` | Measure | 미납품수량 |
| `UNBILLED_QTY` | Measure | 미청구수량 |
| `RDD_LEADTIME_DAYS` | Measure | 리드타임(일) |

### Step A-4. Preview 및 저장

1. **Preview** 버튼 클릭 → 데이터 확인
2. **Save** 클릭 (Ctrl+S)

---

## Part B. Analytic Model 생성

### 이 AM이 표현하는 것

이 Analytic Model의 목적은 **"기준월 대비 기간별 미결 판매오더 현황 비교"** 입니다.

사용자가 기준월(`P_MONTH`, 예: `202504`)을 하나 입력하면, AM이 자동으로 아래 5개 기간을 계산하여 **동일한 지표를 기간별로 나란히 비교**할 수 있게 합니다:

| 구조멤버(Restricted Measure) | 의미 |
|------------------------------|------|
| `Measure_Value` | 전체 (필터 없음) |
| `01_CURR_MONTH` | 당월 (입력한 기준월) |
| `02_PRE_MONTH` | 전월 |
| `03_CURRENT_YEAR_CUMUL` | 당년 1월 ~ 기준월 누계 |
| `04_PRE_YEAR_CUM` | 전년 1월 ~ 전년동기월 누계 |
| `05_PRE_SAME_MONTH` | 전년동기 (전년의 같은 달) |

이를 구현하려면 **Variables(변수)** 를 먼저 정의하고, 각 변수를 이용한 **Restricted Measure(제한 측정값)** 를 구조 멤버로 생성해야 합니다.

---

### Step B-0. V_MONTH_COMPARISON SQL View 생성 (사전 준비)

Analytic Model의 **Calculated Variable**은 외부 SQL View를 참조하여 기간 값을 자동 계산합니다.  
Step B-3에서 생성할 `RV_CURR_MONTH`, `RV_PREVIOUS_MONTH` 등 5개 변수가 모두 `V_MONTH_COMPARISON`을 참조하므로, **AM 생성 전에 반드시 먼저 이 View를 만들어야 합니다.**

#### V_MONTH_COMPARISON이 하는 일

`P_MONTH` 입력 파라미터(예: `202504`)를 받아 아래 5개의 기간 값을 한 번에 계산하여 단일 행으로 반환합니다.

| 출력 컬럼 | 계산 로직 | 예시 (P_MONTH=202504) |
|-----------|-----------|----------------------|
| `CURRENT_MONTH` | 입력값 그대로 | `202504` |
| `PREVIOUS_MONTH` | 전월 (1월이면 전년 12월) | `202503` |
| `CURRENT_YEAR_JAN` | 당년 1월 | `202501` |
| `PREVIOUS_YEAR_SAME_MONTH` | 전년 같은 달 | `202404` |
| `PREVIOUS_YEAR_JAN` | 전년 1월 | `202401` |

> SAP Time Dimension(`SAP.TIME.M_TIME_DIMENSION`)을 소스로 사용하며, `CALMONTH` 기준으로 필터링하여 항상 **1건**만 반환합니다.

#### 생성 절차

1. Data Builder → **New** → **SQL View**
2. 기본 속성 설정:

| 속성 | 값 |
|------|-----|
| Business Name | `Month Comparison` |
| Technical Name | `V_MONTH_COMPARISON` |
| Semantic Usage | `Relational Dataset` |

3. SQL Editor에 아래 쿼리를 입력합니다:

```sql
SELECT DISTINCT
  "M_TIME_DIMENSION_0_0"."CALMONTH"    AS "CURRENT_MONTH",
  "M_TIME_DIMENSION_0_0"."MONTH",
  "M_TIME_DIMENSION_0_0"."CALQUARTER",
  "M_TIME_DIMENSION_0_0"."QUARTER",
  "M_TIME_DIMENSION_0_0"."YEAR",
  CASE
    WHEN RIGHT("M_TIME_DIMENSION_0_0"."CALMONTH", 2) = '01'
    THEN CONCAT(
           CAST(CAST(LEFT("M_TIME_DIMENSION_0_0"."CALMONTH", 4) AS INTEGER) - 1
                AS NVARCHAR(5000)),
           '12')
    ELSE CONCAT(
           LEFT("M_TIME_DIMENSION_0_0"."CALMONTH", 4),
           RIGHT(
             CONCAT('0',
               CAST(CAST(RIGHT("M_TIME_DIMENSION_0_0"."CALMONTH", 2) AS INTEGER) - 1
                    AS NVARCHAR(5000))),
             2))
  END                                  AS "PREVIOUS_MONTH",
  CONCAT(LEFT("M_TIME_DIMENSION_0_0"."CALMONTH", 4), '01')
                                       AS "CURRENT_YEAR_JAN",
  CONCAT(
    CAST(CAST(LEFT("M_TIME_DIMENSION_0_0"."CALMONTH", 4) AS INTEGER) - 1
         AS NVARCHAR(5000)),
    RIGHT("M_TIME_DIMENSION_0_0"."CALMONTH", 2))
                                       AS "PREVIOUS_YEAR_SAME_MONTH",
  CONCAT(
    CAST(CAST(LEFT("M_TIME_DIMENSION_0_0"."CALMONTH", 4) AS INTEGER) - 1
         AS NVARCHAR(5000)),
    '01')                              AS "PREVIOUS_YEAR_JAN"
FROM "SAP.TIME.M_TIME_DIMENSION" AS "M_TIME_DIMENSION_0_0"
WHERE "M_TIME_DIMENSION_0_0"."CALMONTH" = :P_MONTH;
```

> **`PREVIOUS_MONTH` 계산 로직 설명**:  
> 월이 `01`이면 연도에서 1을 빼고 `12`를 붙입니다 (예: `202501` → `202412`).  
> 그 외에는 월에서 1을 빼되 한 자리 월은 앞에 `0`을 붙여 2자리를 유지합니다 (예: `202504` → `202503`).

4. **Input Parameter** 설정:  
   SQL의 `:P_MONTH`에 대응하는 Input Parameter를 추가합니다.

| 속성 | 값 |
|------|-----|
| Technical Name | `P_MONTH` |
| Business Name | `기준월` |
| Data Type | `NVARCHAR` |
| Length | `6` |

5. **Save** 클릭 (Technical Name `V_MONTH_COMPARISON` 확인)

> ✅ 이 View가 정상적으로 저장되어야 Step B-3의 Calculated Variable 생성 시 **Referenced View** 목록에 표시됩니다.

---

### Step B-1. Analytic Model 생성

1. Data Builder → **New** → **Analytic Model**
2. 기본 속성 입력:

| 속성 | 값 |
|------|-----|
| Business Name | `Open Order ODP AM` |
| Technical Name | `WS_RL_OPEN_ORDER_ODP` |

### Step B-2. Fact Source 연결

Add Source 버튼 클릭 → `WS_HL_SQLV_OPEN_ORDER_ODP` 검색 후 선택

Fact View의 수치 필드(KWMENG, GI_QTY 등)가 자동으로 Measures로 인식됩니다.

---

### Step B-3. Variables(변수) 생성

Variables는 AM 실행 시점에 사용자로부터 값을 받거나 다른 변수를 참조하여 자동 계산되는 파라미터입니다.  
Restricted Measure의 필터 조건에서 이 변수들을 참조합니다.

상단 **Variables** 패널 → **+** 버튼으로 아래 6개를 순서대로 생성합니다.

#### (1) P_MONTH — 기준월 입력 변수

사용자가 직접 입력하는 유일한 변수입니다. 나머지 5개는 이 값을 기반으로 자동 계산됩니다.

| 속성 | 값 |
|------|-----|
| Technical Name | `P_MONTH` |
| Business Name | `기준월` |
| Type | `String` |
| Length | `6` |
| Default Value | `202501` |

#### (2) RV_CURR_MONTH — 당월

| 속성 | 값 |
|------|-----|
| Technical Name | `RV_CURR_MONTH` |
| Business Name | `당월` |
| Type | `Calculated Variable` |
| Referenced View | `V_MONTH_COMPARISON` |
| Input Parameter | `P_MONTH` → `P_MONTH` 매핑 |
| Output Parameter | `RV_CURR_MONTH` |

#### (3) RV_PREVIOUS_MONTH — 전월

| 속성 | 값 |
|------|-----|
| Technical Name | `RV_PREVIOUS_MONTH` |
| Business Name | `전월` |
| Type | `Calculated Variable` |
| Referenced View | `V_MONTH_COMPARISON` |
| Output Parameter | `RV_PREVIOUS_MONTH` |

#### (4) RV_CURR_YEAR_JAN — 당년 1월

| 속성 | 값 |
|------|-----|
| Technical Name | `RV_CURR_YEAR_JAN` |
| Business Name | `당년 1월` |
| Type | `Calculated Variable` |
| Referenced View | `V_MONTH_COMPARISON` |
| Output Parameter | `RV_CURR_YEAR_JAN` |

#### (5) RV_PREVIOUS_YEAR_SAME_MONTH — 전년동기월

| 속성 | 값 |
|------|-----|
| Technical Name | `RV_PREVIOUS_YEAR_SAME_MONTH` |
| Business Name | `전년동기월` |
| Type | `Calculated Variable` |
| Referenced View | `V_MONTH_COMPARISON` |
| Output Parameter | `RV_PREVIOUS_YEAR_SAME_MONTH` |

#### (6) RV_PREVIOUS_YEAR_JAN — 전년 1월

| 속성 | 값 |
|------|-----|
| Technical Name | `RV_PREVIOUS_YEAR_JAN` |
| Business Name | `전년 1월` |
| Type | `Calculated Variable` |
| Referenced View | `V_MONTH_COMPARISON` |
| Output Parameter | `RV_PREVIOUS_YEAR_JAN` |

> **참고**: `V_MONTH_COMPARISON`은 `P_MONTH` 값을 받아 전월/당년1월/전년동기 등을 계산하는 표준 계산 뷰입니다.  
> Calculated Variable 생성 시 (2)~(6)은 모두 동일한 `V_MONTH_COMPARISON`을 참조하고 각각 다른 Output Parameter만 선택합니다.

---

### Step B-4. Restricted Measures(제한 측정값) 생성

Restricted Measure는 **Base Measure에 변수 기반 필터를 씌운 새로운 지표**입니다.  
`CalendarYearMonth` 필드를 필터 기준으로, 위에서 만든 변수들을 조건값으로 사용합니다.

Structure Members 패널 → **+ Add Restricted Measure** 로 아래 6개를 생성합니다.

#### Measure_Value — 전체 (필터 없음)

| 속성 | 값 |
|------|-----|
| Technical Name | `Measure_Value` |
| Business Name | `전체` |
| Base Measure | `KWMENG` (또는 분석하려는 지표 선택) |
| Filter | (없음 — 제한 조건 없이 전체 포함) |

> **Expression 입력 방법**: 각 Restricted Measure의 Expression 필드에 아래 조건식을 **직접 텍스트로 입력**합니다.  
> 변수는 `:변수명` 형식으로 참조합니다. **Insert Values** 버튼으로 사용 가능한 변수/필드 목록을 조회해 삽입할 수 있습니다.  
> 작성 후 **Validate** 버튼으로 문법 오류를 확인하세요.

#### 01_CURR_MONTH — 당월

| 속성 | 값 |
|------|-----|
| Technical Name | `01_CURR_MONTH` |
| Business Name | `당월` |
| Base Measure | `KWMENG` |
| Expression | `CalendarYearMonth = :RV_CURR_MONTH` |

#### 02_PRE_MONTH — 전월

| 속성 | 값 |
|------|-----|
| Technical Name | `02_PRE_MONTH` |
| Business Name | `전월` |
| Base Measure | `KWMENG` |
| Expression | `CalendarYearMonth = :RV_PREVIOUS_MONTH` |

#### 03_CURRENT_YEAR_CUMUL — 당년 누계

| 속성 | 값 |
|------|-----|
| Technical Name | `03_CURRENT_YEAR_CUMUL` |
| Business Name | `당년 누계` |
| Base Measure | `KWMENG` |
| Expression | `CalendarYearMonth >= :RV_CURR_YEAR_JAN AND CalendarYearMonth <= :RV_CURR_MONTH` |

#### 04_PRE_YEAR_CUM — 전년 누계

| 속성 | 값 |
|------|-----|
| Technical Name | `04_PRE_YEAR_CUM` |
| Business Name | `전년 누계` |
| Base Measure | `KWMENG` |
| Expression | `CalendarYearMonth >= :RV_PREVIOUS_YEAR_JAN AND CalendarYearMonth <= :RV_PREVIOUS_YEAR_SAME_MONTH` |

#### 05_PRE_SAME_MONTH — 전년동기

| 속성 | 값 |
|------|-----|
| Technical Name | `05_PRE_SAME_MONTH` |
| Business Name | `전년동기` |
| Base Measure | `KWMENG` |
| Expression | `CalendarYearMonth = :RV_PREVIOUS_YEAR_SAME_MONTH` |

> **핵심 원리**: 동일한 `KWMENG`(주문수량)에 서로 다른 기간 필터를 씌운 6개의 지표를 만들었습니다.  
> SAC나 Preview에서 이 6개를 한 화면에 펼치면 **당월/전월/누계/전년동기를 한눈에 비교**할 수 있습니다.

---

### Step B-5. 저장 및 Preview

1. **Save** 클릭 (Technical Name `WS_RL_OPEN_ORDER_ODP` 확인)
2. **Preview** 클릭
3. Variables 입력 창에서 `P_MONTH` 값 입력 (예: `202501`)
4. Structure Members에서 `01_CURR_MONTH` ~ `05_PRE_SAME_MONTH` 선택 후 결과 확인

**검증 포인트:**

| 확인 항목 | 기대 결과 |
|----------|---------|
| Variables 패널 | `P_MONTH` + `RV_*` 5개 총 6개 표시 |
| Structure Members 패널 | `Measure_Value` + `01`~`05` 총 6개 표시 |
| Preview — 기간별 수치 | 당월/전월/누계가 서로 다른 값으로 표시 |

---

### (참고) CSN Import로 완성본 불러오기

직접 구성이 어려운 경우, 미리 준비된 CSN JSON을 Import하여 완성된 AM을 사용할 수 있습니다.

<details>
<summary>CSN Import 방법 보기</summary>

1. 기존에 직접 만든 `WS_RL_OPEN_ORDER_ODP`가 있으면 **먼저 삭제**합니다  
   (동일 Technical Name이 존재하면 Import가 Skip됩니다)
2. Data Builder → 우상단 **Import** 버튼(↓ 아이콘) 클릭
3. **Import Objects from CSN/JSON File** 선택
4. 파일 선택: `WS_RL_OPEN_ORDER_ODP.json`
5. Import 완료 후 `WS_RL_OPEN_ORDER_ODP` AM 오픈하여 구조 확인

</details>

---

## 전체 데이터 흐름

```mermaid
flowchart LR
    subgraph S4H["S/4HANA (HE4)"]
        ODP1["2LIS_11_VAHDR\n수주헤더"]
        ODP2["2LIS_11_VAITM\n수주아이템"]
        ODP3["2LIS_11_VASCL\n납품일정행"]
        ODP4["2LIS_12_VCITM\n납품아이템"]
        ODP5["2LIS_13_VDITM\n청구아이템"]
        ODP6["2LIS_13_VDHDR\n청구헤더"]
    end

    subgraph DSP["SAP Datasphere"]
        LT1["SAP_SD_IL_2LIS_11_VAHDR\n(Local Table)"]
        LT2["SAP_SD_IL_2LIS_11_VAITM\n(Local Table)"]
        LT3["SAP_SD_IL_2LIS_11_VASCL\n(Local Table)"]
        LT4["SAP_SD_IL_2LIS_12_VCITM\n(Local Table)"]
        LT5["SAP_SD_IL_2LIS_13_VDITM\n(Local Table)"]
        LT6["SAP_SD_IL_2LIS_13_VDHDR\n(Local Table)"]
        FV["WS_HL_SQLV_OPEN_ORDER_ODP\n(SQL View / Fact)"]
        AM["WS_RL_OPEN_ORDER_ODP\n(Analytic Model)"]
    end

    ODP1 -->|"Replication Flow"| LT1
    ODP2 -->|"Replication Flow"| LT2
    ODP3 -->|"Replication Flow"| LT3
    ODP4 -->|"Replication Flow"| LT4
    ODP5 -->|"Replication Flow"| LT5
    ODP6 -->|"Replication Flow"| LT6

    LT1 & LT2 & LT3 & LT4 & LT5 & LT6 -->|"JOIN + 집계"| FV
    FV --> AM
```

---

## 완료 체크리스트

- [ ] `WS_HL_SQLV_OPEN_ORDER_ODP` SQL View 생성 (Semantic Usage: Fact)
- [ ] SQL 정상 실행 확인 (Run 버튼)
- [ ] Key/Measure/Attribute Semantic Type 설정
- [ ] Preview 데이터 확인
- [ ] `WS_RL_OPEN_ORDER_ODP` Analytic Model 생성 및 Fact Source 연결
- [ ] Variables 6개 생성 (P_MONTH + RV_* 5개)
- [ ] Restricted Measures 6개 생성 (Measure_Value + 01~05)
- [ ] P_MONTH 입력 후 Preview — 기간별 수치 비교 확인

---

다음 단계 → **[Lab 4-B: CDS View 기반 모델 개발](./lab4b-cdsv-fact-view-am.md)**
