...
**업종**: 제조업 (SAP S/4HANA 운영 중, 시스템 코드: HKQ)
...
| 시스템 | 테이블 | 설명 | 비고 |
|--------|--------|------|------|
| HKQ | `VBAK` | Sales Document Header | 기준 헤더 |
| HKQ | `VBAP` | Sales Document Item | JOIN: VBELN |
| HKQ | `VBEP` | Sales Document Schedule Line | JOIN: VBELN+POSNR, VBAP 레벨 집계 후 매칭 |
| HKQ | `VBUK` | Sales Document: Header Status | 상태 정보 |
| HKQ | `VBUP` | Sales Document: Item Status | 상태 정보 |
| HKQ | `LIKP` | Delivery Header Data | JOIN: VBELN(LIPS-VBELN) |
| HKQ | `LIPS` | Delivery Item Data | JOIN: VGBEL=VBAP-VBELN, VGPOS=VBAP-POSNR |
| HKQ | `VBRK` | Billing Document Header | JOIN: VBELN(VBRP) |
| HKQ | `VBRP` | Billing Document Item | JOIN: AUBEL=VBAP-VBELN, AUPOS=VBAP-POSNR |
| HKQ | `KNA1` | Customer Master | JOIN: KUNNR=VBAK-KUNNR |
| HKQ | `ZSTT00020` | 자재 분류정보 (사이즈/패턴/종류 등) | JOIN: MATNR=VBAP-MATNR |
...
청구 기준의 SD Sell-in 실적 현황을 분석. ZSDM80210 프로그램 로직 기반.

### AS-IS 소스 테이블

| 시스템 | 테이블 | 설명 |
|--------|--------|------|
| HKQ | `VBRK` | Billing Document Header |
| HKQ | `VBRP` | Billing Document Item |
| HKQ | `VBAK` | Sales Document Header (참조) |
| HKQ | `KNA1` | Customer Master |
| HKQ | `ZSTT00020` | 자재 분류정보 |
| HKQ | `ZSDT02073` | OE/RE 구분 및 비즈니스 유형 판정 테이블 |
...
### 한국 법인(BUKRS = '1100') 내수/수출 구분 처리

| 구분 | 유통채널 | 제외 판매전표유형 |
|------|----------|----------------|
| 내수 | `VTWEG IN ('01','04','06')` | ZIH1, ZSR1, ZDN1 또는 특정 청구유형 외 |
| 수출 | `VTWEG IN ('02','05')` | ZRE1, ZEX1, ZSD1, ZREN, ZCR1, ZCRR, ZDR1, ZDRR, ZCP1, ZWH1, ZWH2, ZSD2 외 |
...
| `BUKRS` / `BillingCompanyCode` | 회사코드 | `2LIS_11_VAHDR.BUKRS` | `C_SalesDocumentItemDEX_1.BillingCompanyCode` |
