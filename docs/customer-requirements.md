...
> **"주문 접수 대비 재고 Confirmed 수량 및 출하 진행량을 실시간으로 파악하고, 권역별 재고를 선제적으로 조달하여 납기 준수 및 빠른 매출로 이어지도록 하기 위함"**
...
| HE4 | `MARA` | Material Master - General Data | JOIN: MATNR=VBAP-MATNR |
...
> **"판매문서의 납품요청일(RDD) 대비 실제 출하일 기준으로 납기 준수 여부를 판단하여, 지연 오더를 조기에 식별하여 즉시 액션을 취할 수 있도록 한다."**
